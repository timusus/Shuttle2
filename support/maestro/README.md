...
| `support/scripts/checks/shell-sheet-levels.sh` | receivers + `shell-sheet-levels.yaml` | The debug Compose shell (#375): the player sheet steps Mini -> Now Playing -> Queue by taps and a drag, back steps it down one level at a time, then pops the destination | ~60 s |
| `support/scripts/checks/shell-player.sh` | receivers + `shell-player.yaml` | The debug Compose shell's player (#376): expand, play/pause, seek, skip, then swipe a song out of the queue, drag one to the top by its handle and tap one to play it; playback follows the new order | ~45 s |
...
| `support/scripts/checks/shell-home-search.sh` | receivers + `shell-home-search.yaml` | The debug Compose shell's Home and Search (#377): Home shows the library and a Home album opens its album screen, its search action opens Search, an artist result opens its artist screen, and tapping a song result plays it | ~50 s |
| `support/scripts/checks/shell-tag-editor.sh` | `taglib` provider + `shell-tag-editor.yaml` | The debug Compose shell's song info and tag editor (#377): a song's info opens from its row menu, and a title edited from a one-song selection's Edit Tags shows in the Songs tab once written | ~90 s |
