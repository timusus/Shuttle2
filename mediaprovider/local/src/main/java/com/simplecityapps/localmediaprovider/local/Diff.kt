...
class Diff<T>(val updates: List<Pair<T, T>>, val insertions: List<T>, val deletions: List<T>, val unchanged: List<Pair<T, T>>) {
    ...
    companion object {
         ...
         */
        fun <T : ContentsComparator<T>> diff(
            oldData: List<T>,
            newData: List<T>
        ): Diff<T> {
            ...
            val matches = newData.mapNotNull { newItem ->
                oldData.firstOrNull { oldItem -> oldItem == newItem }?.let { oldItem ->
            ...
            val updates = matches.filterNot { match -> match.first.areContentsEqual(match.second) }
