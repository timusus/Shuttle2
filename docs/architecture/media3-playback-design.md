...
| System audio effects session | AudioEffectSessionManager | Player.Listener.onAudioSessionIdChanged + ExoPlayer.setAudioSessionId | Thin listener; rebinding on player switch goes away |
...
| Audio focus + ducking | AudioFocusHelper* (audiofocus/AudioFocusHelperBase.kt:36-90) | ExoPlayer.setAudioAttributes(attrs, handleAudioFocus = true) | Delete. Check that focus is abandoned on pause (ea250913) (unverified). |
| Becoming noisy | NoiseManager | ExoPlayer.setHandleAudioBecomingNoisy(true) | Delete |
...
- PlaybackManager, QueueManager and LoadCoordinator are deleted.
