...
| Resolving remote URIs | suspend MediaResolver (exoplayer/MediaResolver.kt), Jellyfin auth (JellyfinMediaInfoProvider.getMediaInfo) | Playlist items keep their jellyfin://, emby://, plex:// URIs. A ResolvingDataSource.Factory resolves them when opened: the loader thread waits (`awaitBlocking`) on a resolution running in the resolver's own scope, shared by every open of that URI. | The resolver itself. StreamSniffingMediaSourceFactory is kept. |
...
| Position, progress and anchor | progressFlow, positionAnchorFlow, ProgressTicker | Player.currentPosition + isPlaying + playbackParameters; controllers get position from the session | Done (#345): ProgressTicker derives progressFlow from the Player; positionAnchorFlow is deleted, and the speed the player shows comes from playbackSpeedFlow |
...
3. **Platform concerns.** ExoPlayer handles audio focus and becoming noisy. SleepTimer, BitPerfectOutput, trial speed and PlaybackReporter listen to the Player. Deletes: AudioFocusHelper*, NoiseManager, AudioEffectSessionManager, and whatever is left of PlaybackManager and QueueManager. Supersedes #250 and #327. Verify: RS focus and USB tests, USB DAC device checks.
   *Status:* focus, becoming noisy and the effects session landed. AudioFocusHelper* and NoiseManager are deleted; S2's focus behaviour is Media3's (RS-50 to RS-53), with one change: pausing keeps focus, as Android asks, where S2 used to give it up (RS-04). Media3 takes the delayed focus a phone call gives as focus, so a play during a call is held until the call ends, through CallMonitor (RS-54). SleepTimer, trial speed and PlaybackReporter already listen to PlaybackOperations' flows, which derive from the Player; BitPerfectOutput listens to the AudioTrack through AudioTrackMonitor.
   *The remainder, in three parts.* What's left in PlaybackManager and QueueManager isn't forwarding, so each concern moves to a class of its own that listens to the Player, or goes where Media3 already covers it:
