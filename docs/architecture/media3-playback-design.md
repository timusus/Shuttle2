...
| Audio focus + ducking | AudioFocusHelper* (audiofocus/AudioFocusHelperBase.kt:36-90) | ExoPlayer.setAudioAttributes(attrs, handleAudioFocus = true) | Done (step 3). Media3 holds focus while set to play and doesn't give it up on pause, so ExoPlayerFactory turns focus handling off and on again on each pause, which abandons it (RS-04). |
