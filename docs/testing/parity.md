...
| Jellyfin, Emby, Plex: connect, edit, remember password, errors with retry, remove source, report playback | Still the View `*ConfigurationFragment` dialogs, via `screens/sources/servers/ServerSignIn` | `JellyfinAuthenticationTest`, `EmbyAuthenticationTest`, `*PlaybackReporterTest`; dialogs untested | Compose rebuild: #434 |
