...
interface TagEditorContract {
    ...
    interface Presenter : BaseContract.Presenter<View> {
        fun load(songs: List<com.simplecityapps.shuttle.model.Song>)
        fun save(data: Data)
    }
}
...
class TagEditorPresenter @Inject constructor(
    ...
    override fun load(songs: List<com.simplecityapps.shuttle.model.Song>) {
        view?.setLoading(TagEditorContract.LoadingState.ReadingTags(0, songs.size))

        launch {
            val songAudioFilePairs = songs.mapIndexed { index: Int, song: com.simplecityapps.shuttle.model.Song ->
                ...
                editables.mapIndexed { index, (song, _) ->
                    songRepository.update(
                        song.copy(
                            ...
                            date = if (data.dateField.hasChanged) data.dateField.currentValue?.toIntOrNull()?.let { LocalDate(it, 1, 1) } else song.date,
