...
unsigned int ID3v2::Tag::trackTotal() const {
    if (!d->frameListMap["TRCK"].isEmpty()) {
        const StringList &stringList = d->frameListMap["TRCK"].front()->toString().split("/");
        if (!stringList.isEmpty()) {
            return stringList.back().toInt();
        }
    }
    return 0;
}

String ID3v2::Tag::trackTot() const {
    if(!d->frameListMap["TRCK"].isEmpty()) {
        const StringList &stringList = d->frameListMap["TRCK"].front()->toString().split("/");
        if(!stringList.isEmpty()) {
