...
class DirectoryAlbumArtistLocalArtworkModelLoader(
    ...
    class DirectoryAlbumArtistLocalArtworkProvider(
        ...
        override suspend fun getInputStream(): InputStream? = songRepository.getSongs(SongQuery.ArtistGroupKeys(listOf(SongQuery.ArtistGroupKey(albumArtist.groupKey))))
            .firstOrNull()
            ?.firstOrNull()
            ?.let { song ->
