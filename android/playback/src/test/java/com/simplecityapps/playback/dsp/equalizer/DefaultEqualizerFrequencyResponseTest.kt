...
class DefaultEqualizerFrequencyResponseTest {
    ...
    fun `a flat preset produces a response that never exceeds 0 dB`() {
        ...
        val points = computeFrequencyResponse(bands, outputSampleRateHz = 44_100, MIN_FREQUENCY_HZ, MAX_FREQUENCY_HZ, pointCount = 240)
    ...
    fun `falls back to a named sample rate when the processor hasn't configured an output format`() {
        ...
        val withNullSampleRate = computeFrequencyResponse(bands, outputSampleRateHz = null, MIN_FREQUENCY_HZ, MAX_FREQUENCY_HZ, pointCount = 240)
        val withFallbackSampleRate = computeFrequencyResponse(bands, outputSampleRateHz = FALLBACK_OUTPUT_SAMPLE_RATE_HZ, MIN_FREQUENCY_HZ, MAX_FREQUENCY_HZ, pointCount = 240)
    ...
    fun `the plotted range extends to Nyquist of the given output sample rate`() {
        ...
        val points = computeFrequencyResponse(bands, outputSampleRateHz = 16_000, MIN_FREQUENCY_HZ, MAX_FREQUENCY_HZ, pointCount = 240)
