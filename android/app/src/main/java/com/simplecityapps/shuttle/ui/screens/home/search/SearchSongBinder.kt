...
open class SearchSongBinder(
    ...
    class ViewHolder(itemView: View) : ViewBinder.ViewHolder<SearchSongBinder>(itemView) {
        ...
        override fun bind(
            ...
            val query = viewBinder.query
            title.text = query.highlight(viewBinder.song.name ?: unknown, accentColor)
            subtitle.text =
                listOfNotNull(viewBinder.song.friendlyArtistName ?: viewBinder.song.albumArtist, viewBinder.song.album)
                    .joinToSpannedString(" • ") { query.highlight(it, accentColor) }
                    .ifEmpty { unknown }
        }
